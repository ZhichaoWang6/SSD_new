...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    def __init__(self, config: Qwen2_5_VLConfig):
        ...
        self.gradient_checkpointing = False
        self.all_last_frames = None     # used for DTD during inference, when a new turn with frames comes in
        self.all_keep_masks = list()
        self.all_drop_ratios = list()
        print("model load debug: loaded all keep masks for Qwen2_5_VLModel")
    ...
    def token_drop(
        ...
        for i in range(batch_size):
            ...
            drop_pos = {}
            all_last_frames = list()
            drop_ratio_per_frame = list()
            ...
            if len(visual_token_indices) > 0:
                ...
                if method == 'feature':
                    curr_frame = None
                    # prev_frame 改成直接从调用这个函数的地方传入
                    # prev_frame = None
                    prev_frame = self.all_last_frames[i] if self.all_last_frames is not None else None
                    idx_this_frame = -1
                    for t in unique_frames:
                        frame_indices = (time_ids == t).nonzero(as_tuple=True)[0]
                        frame_indices = [idx for idx in frame_indices if idx.item() in visual_token_indices]
                        ...
                        # drop_ratio_per_frame.append(f"{frame_indices}, {t}, {len(drop_indices_this_frame)} / {len(frame_indices)}")
                        drop_ratio_per_frame.append(len(drop_indices_this_frame) / len(frame_indices))
            ...
            all_last_frames.append(curr_frame.unsqueeze(0))
            keep_mask_list.append(keep_mask.unsqueeze(0))
            drop_ratio_list.append(drop_ratio_per_frame)
        ...
        self.all_last_frames = all_last_frames
        ...
        return dropped_hidden_states, dropped_position_embeddings, dropped_pos_ids, \
            {'keep_mask': keep_mask, 'drop_ratio': drop_ratio_list}

    # NOT USED NOW
    def token_drop_single_frame(
            self,
            threshold: float,
        ...
        """
        this func only works at feature space, not pixel space
        all tensor inputs are without batch dimension
        Args:
            curr_frame: [num_patches, hidden_dim]
            prev_frame: [num_patches, hidden_dim] (before dropping)
        
        Returns:
            dropped_curr_frame: [num_patches_after_drop, hidden_dim]
            dropped_position_embeddings: (
                [3, batch_size, new_seq_len, emb_dim],
                [3, batch_size, new_seq_len, emb_dim]
            )
            dropped_position_ids: [3, new_seq_len]
        """
        seq_len = curr_frame.size(0)
        keep_mask = torch.ones(seq_len, dtype=torch.bool, device=curr_frame.device)
        similarity = F.cosine_similarity(prev_frame, curr_frame, dim=1) # [num_patches]
        if absolute:
            drop_indices_this_frame = (similarity > threshold).nonzero(as_tuple=True)[0]
        else:
            k = int(similarity.numel() * threshold)
            drop_indices_this_frame = torch.topk(similarity, k).indices
        keep_mask[drop_indices_this_frame] = False

        dropped_curr_frame = curr_frame[keep_mask]
        pos_emb1_i = position_embeddings[0]  # shape [3, seq_len, emb_dim]
        pos_emb2_i = position_embeddings[1]  # same shape
        dropped_pos_emb1 = pos_emb1_i.transpose(0,1)[keep_mask.to(pos_emb1_i.device)].transpose(0,1)
        dropped_pos_emb2 = pos_emb2_i.transpose(0,1)[keep_mask.to(pos_emb2_i.device)].transpose(0,1)
        dropped_pos_ids = position_ids.transpose(0,1)[keep_mask.to(position_ids.device)].transpose(0,1)  # shape [3, new_seq_len]
        dropped_position_embeddings = (dropped_pos_emb1, dropped_pos_emb2)

        return dropped_curr_frame, dropped_position_embeddings, dropped_pos_ids
    ...
    def forward(
        ...
        if drop_method is not None and drop_method.lower() != 'none' \
            ...
            hidden_states, position_embeddings, position_ids, more_info = self.token_drop(
                method=drop_method,
            ...
            keep_mask, drop_ratio = more_info['keep_mask'], more_info['drop_ratio']
            self.all_keep_masks.append(keep_mask)
            if len(self.all_drop_ratios) == 0:
                self.all_drop_ratios = drop_ratio
            else:
                for i in range(len(self.all_drop_ratios)):
                    self.all_drop_ratios[i] += drop_ratio[i]
        else:
            # 如果根本没进行drop（比如这轮input_ids中没有图片token）
            self.all_keep_masks.append(torch.ones_like(input_ids, dtype=torch.bool))
            drop_ratio = None
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def reset_status(self):
        self.model.all_last_frames = None
        self.model.all_keep_masks = list()
        self.model.all_drop_ratios = list()
    ...
    def get_rope_index(
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                position_ids[..., i, attention_mask[i] == 1] = llm_positions.to(position_ids.device).to(position_ids.dtype)
    ...
    def forward(
        ...
        # position_ids.size(1) != 3: this position_ids is given by some codebase that do not handle rope position_ids differently for Qwen2.5-VL
        if (position_ids is None or position_ids.size(1) != 3) and \
            ...
            # then use the prev pre-calculated rope-deltas to get the correct position ids
                ...
                # if the new input_ids does not contain visual content, use default auto-increase position ids
                image_nums, video_nums = self._get_image_nums_and_video_nums(input_ids)
                if image_nums == 0 and video_nums == 0:
                    # print('no visual content in new input_ids, use default auto-increase position ids...')
                    ...
                    # ⬆️ we can't simply use torch.arange as new position ids, as the new input tokens may contain images/videos.
                    # we need to use self.get_rope_index to calculate position ids for the new tokens.
                    # print('getting position ids for new input tokens...')
                    position_ids, rope_deltas = self.get_rope_index(
                        input_ids,
                        image_grid_thw,
                        video_grid_thw,
                        second_per_grid_ts,
                        # input_ids was left-truncated by prepare_inputs_for_generation(), but attention_mask was not.
                        attention_mask[:, -seq_length:],
                    )
                    position_ids = position_ids.add(delta)
                    self.rope_deltas += rope_deltas

        # debug: check the new position ids
        # print(f"{input_ids.tolist()=}")
        # print(f"{position_ids.tolist()=}")

        # if position_ids is not None and position_ids.shape[-1] <= 2:
        #     print(f"[FULL_POS_DEBUG] position_ids={position_ids[:, 0, :]}")
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if cache_position[0] != 0:
            # 检查后续的input_ids还有多少个image和video
            image_nums, video_nums = self._get_image_nums_and_video_nums(input_ids)
            image_nums, video_nums = image_nums.item(), video_nums.item()

            if image_nums == 0:
                pixel_values = None
            else:
                image_grid_thw = image_grid_thw[-image_nums:]
                pixel_values = pixel_values[-sum([i.prod() for i in image_grid_thw]):]
            
            if video_nums == 0:
                pixel_values_videos = None
            else:
                video_grid_thw = video_grid_thw[-video_nums:]
                pixel_values_videos = pixel_values_videos[-sum([i.prod() for i in video_grid_thw]):]
...
class Qwen2_5_VL_DTDForConditionalGeneration(Qwen2_5_VLForConditionalGeneration):
    pass

__all__ = ["Qwen2_5_VLForConditionalGeneration", "Qwen2_5_VL_DTDForConditionalGeneration", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel"]
